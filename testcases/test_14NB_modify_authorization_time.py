# -*- coding:utf-8 -*-
# @Time   : 2021/1/20 15:57
# @Author :sleep
# @Email  :1285592010@qq.com
# @File   : test_14NB_modify_authorization_time.py
# @Software:PyCharm
# @File name Translation:修改授权时间
import unittest
import os
import requests
import time
...
@ddt
class TestNBmodityAuthorizationTime(unittest.TestCase):
    ...
    #---------------------------------------------------
    excel=HandleExcel(os.path.join(DATA_DIR,"apicases.xlsx"),"14_nb_modify_authorization_t1")
    #读取用例数据
    ...
    start_time_after_modification=conf.get("time","start_time_after_modification")
    end_time_after_modification=conf.get("time","end_time_after_modification")
    #实际门锁类型获取
    ...
    def test_nb_modify_authorization_time(self,item):
        ...
        if lock_type<=self.practical_lock_type:
            ...
            if "#start_time_after_modification#" in item["data"]:
                ...
                item["data"] = item["data"].replace("#start_time_after_modification#",start_time_after_modification)
            if "#end_time_after_modification#" in item["data"]:
                end_time_after_modification=self.end_time_after_modification
                item["data"] = item["data"].replace("#end_time_after_modification#",end_time_after_modification)
            ...
            if item["title"] == "NB授权-普通用户新增-密码":
                TestNBmodityAuthorizationTime.userId= jsonpath(res, "$..userId")[0]
                TestNBmodityAuthorizationTime.usePwId= jsonpath(res, "$..usePwId")[0]
            #第三部：断言
            try:
