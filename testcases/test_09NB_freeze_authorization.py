# -*- coding:utf-8 -*-
# @Time   : 2021/1/20 15:46
# @Author :sleep
# @Email  :1285592010@qq.com
# @File   : test_09NB_freeze_authorization.py
# @Software:PyCharm
# @File name Translation:冻结授权（还需要看一下，为什么门锁会一闪一闪的YAYA）
import unittest
import os
import requests
import time
...
@ddt
class TestNBfreezeAuthorization(unittest.TestCase):
    ...
    #---------------------------------------------------
    excel=HandleExcel(os.path.join(DATA_DIR,"apicases.xlsx"),"09_freeze(thaw)_authorization")
    #读取用例数据
    ...
    def test_nb_authorization_list_details(self,item):
        ...
        if lock_type<=self.practical_lock_type:
            ...
            res=response.json()
            #提取返回出来的参数
            if item["title"] == "NB授权-临时用户新增-密码":
                TestNBfreezeAuthorization.userId= jsonpath(res, "$..userId")[0]
                TestNBfreezeAuthorization.usePwId= jsonpath(res, "$..usePwId")[0]
            #第三部：断言
            try:
