...
# @File   : test_12NB_freezing_locks.py
# @Software:PyCharm
# @File name Translation:冻结门锁(家里面测试门锁不支持)
import unittest
import os
import requests
import time
...
@ddt
class TestNBfreezeLooks(unittest.TestCase):
    ...
    #---------------------------------------------------
    excel=HandleExcel(os.path.join(DATA_DIR,"apicases.xlsx"),"12_freezing(defrost_door)_lock")
    #读取用例数据
    ...
    @list_data(cases)
    def test_nb_authorization_looks(self,item):
        #门锁类型获取
